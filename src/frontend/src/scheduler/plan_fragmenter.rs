...
impl BatchPlanFragmenter {
    ...
    fn split_into_stage(&mut self, batch_node: PlanRef) -> SchedulerResult<()> {
        let root_stage_id = self.new_stage(
            batch_node,
...
impl QueryStage {
    ...
    pub fn with_exchange_info(
        ...
        if let Some(exchange_info) = exchange_info {
            Self {
                ...
                file_scan_info: self.file_scan_info,
                ...
                children_exchange_distribution: self.children_exchange_distribution,
            }
        } else {
            self
        }
    ...
    pub fn with_exchange_info_and_complete_source_info(
        self,
        exchange_info: Option<ExchangeInfo>,
...
struct QueryStageBuilder {
    id: StageId,
    root: Option<ExecutionPlanNode>,
    parallelism: Option<u32>,
    exchange_info: Option<ExchangeInfo>,

    children_stages: Vec<StageId>,
...
impl QueryStageBuilder {
    ...
    fn finish(self, stage_graph_builder: &mut StageGraphBuilder) -> StageId {
        ...
        stage_graph_builder.add_node(stage);
        for child_stage_id in self.children_stages {
            stage_graph_builder.link_to_child(self.id, child_stage_id);
        }
        stage_id
    }
}
...
pub struct StageGraph {
    ...
    pub stages: HashMap<StageId, QueryStage>,
...
enum StageCompleteInfo {
    ExchangeInfo((Option<ExchangeInfo>, Option<u32>)),
    ExchangeWithSourceInfo((Option<ExchangeInfo>, SourceScanInfo, u32)),
}
...
impl StageGraph {
    ...
    async fn complete(
        ...
        Ok(StageGraph {
            ...
            stages: complete_stages
                .into_iter()
                .map(|(stage_id, info)| {
                    let stage = stages.remove(&stage_id).expect("should exist");
                    let stage = match info {
                        StageCompleteInfo::ExchangeInfo((exchange_info, parallelism)) => {
                            stage.with_exchange_info(exchange_info, parallelism)
                        }
                        StageCompleteInfo::ExchangeWithSourceInfo((
                            exchange_info,
                            source_info,
                            parallelism,
                        )) => stage.with_exchange_info_and_complete_source_info(
                            exchange_info,
                            source_info,
                            parallelism,
                        ),
    ...
    async fn complete_stage(
        ...
        let parallelism = if stage.parallelism.is_some() {
            ...
            let complete_stage_info = StageCompleteInfo::ExchangeWithSourceInfo((
                exchange_info,
                complete_source_info,
                task_parallelism,
            ));
            complete_stages.insert(stage.id, complete_stage_info);
            Some(task_parallelism)
        } else {
            ...
            complete_stages.insert(
                stage.id,
                StageCompleteInfo::ExchangeInfo((exchange_info, Some(parallelism as u32))),
            );
            None
        };

        for child_stage_id in self
            .child_edges
            .get(&stage.id)
...
struct StageGraphBuilder {
    stages: HashMap<StageId, QueryStage>,
    child_edges: HashMap<StageId, HashSet<StageId>>,
    parent_edges: HashMap<StageId, HashSet<StageId>>,
    batch_parallelism: usize,
}
...
impl StageGraphBuilder {
    ...
    pub fn add_node(&mut self, stage: QueryStage) {
...
impl BatchPlanFragmenter {
        ...
        match node.node_type() {
            ...
            _ => {
                ...
                if let Some(parent) = parent_exec_node {
                    parent.children.push(execution_plan_node);
                } else {
                    builder.root = Some(execution_plan_node);
                }
            }
        }
        Ok(())
    ...
    fn visit_exchange(
        ...
        let child_stage_id = self.new_stage(node.inputs()[0].clone(), child_exchange_info)?;
        execution_plan_node.source_stage_id = Some(child_stage_id);
        if builder.parallelism.is_none() {
            builder
                .children_exchange_distribution
                .insert(child_stage_id, node.distribution().clone());
        }
        ...
        builder.children_stages.push(child_stage_id);
        Ok(())
    }
...
mod tests {
    ...
    async fn test_fragmenter() {
        ...
        assert_eq!(query.stage_graph.root_stage_id, 0.into());
        assert_eq!(query.stage_graph.stages.len(), 4);

        // Check the mappings of child edges.
        assert_eq!(
            query.stage_graph.child_edges[&0.into()],
            HashSet::from_iter([1.into()])
        );
        assert_eq!(
            query.stage_graph.child_edges[&1.into()],
            HashSet::from_iter([2.into(), 3.into()])
        );
        assert_eq!(query.stage_graph.child_edges[&2.into()], HashSet::new());
        assert_eq!(query.stage_graph.child_edges[&3.into()], HashSet::new());

        // Check the mappings of parent edges.
        assert_eq!(query.stage_graph.parent_edges[&0.into()], HashSet::new());
        assert_eq!(
            query.stage_graph.parent_edges[&1.into()],
            HashSet::from_iter([0.into()])
        );
        assert_eq!(
            query.stage_graph.parent_edges[&2.into()],
            HashSet::from_iter([1.into()])
        );
        assert_eq!(
            query.stage_graph.parent_edges[&3.into()],
            HashSet::from_iter([1.into()])
        );
        ...
        let root_exchange = query.stage_graph.stages.get(&0.into()).unwrap();
        assert_eq!(
            root_exchange.root.node_type(),
            BatchPlanNodeType::BatchExchange
        );
        assert_eq!(root_exchange.root.source_stage_id, Some(1.into()));
        ...
        let join_node = query.stage_graph.stages.get(&1.into()).unwrap();
        ...
        assert_eq!(join_node.root.children[0].source_stage_id, Some(2.into()));
        ...
        assert_eq!(join_node.root.children[1].source_stage_id, Some(3.into()));
        assert_eq!(0, join_node.root.children[1].children.len());
        assert!(!join_node.has_table_scan());

        let scan_node1 = query.stage_graph.stages.get(&2.into()).unwrap();
        ...
        let scan_node2 = query.stage_graph.stages.get(&3.into()).unwrap();
